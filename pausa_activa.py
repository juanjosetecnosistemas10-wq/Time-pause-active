...
DEFAULT_CONFIG = {
    ...
    "meta_pausas": 6,
    # ── NUEVAS OPCIONES ──────────────────────────────
    "no_molestar": True,          # detectar pantalla completa
    "agua_activo": True,          # recordatorio de agua
...
# ══════════════════════════════════════════════════════════════════════════════
# MEJORA 1 — Modo No Molestar (detecta pantalla completa en Windows)
# ══════════════════════════════════════════════════════════════════════════════
def is_fullscreen_app_running():
    """
    Devuelve True si hay una aplicación en pantalla completa activa
    (reunión, presentación, video, juego, etc.).
    Usa la API de Windows: SHQueryUserNotificationState.
    """
    try:
        import ctypes
...
class WaterReminder:
    ...
    def _loop(self):
        while not self._stop.is_set():
            ...
            mins = max(1, cfg.get("agua_min", 30))
            # Espera el intervalo en trozos de 1 seg para poder parar
            for _ in range(mins * 60):
                if self._stop.is_set(): return
                time.sleep(1)
            if self._stop.is_set(): return
            send_win_notification("💧 Hidratación", "¡Recuerda tomar agua!")
            try:
...
def load_stats():
    ...
    if os.path.exists(STATS_FILE):
        ...
        return s
    return {"fecha": today, "completadas": 0, "saltadas": 0,
...
class PausaWindow(tk.Toplevel):
    def __init__(self, parent, ejercicio, duracion_sec, on_done, on_skip, countdown_pct, sonido_ambiente="ninguno"):
        super().__init__(parent)
        self.on_done    = on_done
        self.on_skip    = on_skip
        self.remaining  = duracion_sec
        ...
        self._build(countdown_pct)
        self._center()
        self._tick()
        self.protocol("WM_DELETE_WINDOW", self._skip)

        # Inicia sonido ambiente si está configurado
        if sonido_ambiente != "ninguno":
            start_ambient(sonido_ambiente)
    ...
    def _build(self, pct):
        ...
        self.pb = ttk.Progressbar(self, orient="horizontal", length=340, mode="determinate",
                                  maximum=self.remaining, value=self.remaining)
        s = ttk.Style(self); s.theme_use("default")
        s.configure("g.Horizontal.TProgressbar", troughcolor=BG3, background=GREEN,
                    bordercolor=BG3, lightcolor=GREEN, darkcolor=GREEN)
        self.pb.configure(style="g.Horizontal.TProgressbar")
        self.pb.pack(padx=32, pady=12)
        tk.Button(self, text="Saltar pausa", font=("Segoe UI", 9), bg=BG3, fg=TEXT_DIM,
                  bd=0, cursor="hand2", activebackground=BG3, activeforeground=TEXT,
                  command=self._skip, relief="flat").pack(pady=(0, 24))
...
class StatsWindow(tk.Toplevel):
    def __init__(self, parent, stats, meta):
        super().__init__(parent)
        self.title("Estadisticas"); self.configure(bg=BG)
        ...
        tk.Label(self, text="Estadísticas de hoy", font=("Segoe UI", 13, "bold"), bg=BG, fg=ACCENT).pack(pady=(24, 16))
        frame = tk.Frame(self, bg=BG2, highlightthickness=1, highlightbackground=BORDER)
        ...
        rows = [
            ...
            ("Meta diaria",        "CUMPLIDA" if meta_ok else "En progreso", GREEN if meta_ok else TEXT_DIM),
        ]
        ...
        tk.Button(bf, text="Exportar CSV", font=("Segoe UI", 9), bg=BG3, fg=TEXT, bd=0,
                  cursor="hand2", activebackground=BORDER, activeforeground=TEXT, relief="flat",
                  padx=12, pady=6,
...
class ConfigWindow(tk.Toplevel):
    def __init__(self, parent, cfg, on_save):
        ...
        self.title("Configuracion"); self.configure(bg=BG)
        self.resizable(False, False); self.attributes("-topmost", True)
        self._build(); self._center()

    def _field(self, parent, label, var, row):
        tk.Label(parent, text=label, font=("Segoe UI", 10), bg=BG2, fg=TEXT_DIM, anchor="w").grid(
            row=row, column=0, sticky="w", padx=16, pady=8)
        tk.Entry(parent, textvariable=var, font=("Segoe UI", 11), bg=BG3, fg=TEXT,
    ...
    def _section(self, text):
        tk.Frame(self, bg=BORDER, height=1).pack(fill="x", padx=28, pady=(12, 4))
        tk.Label(self, text=text, font=("Segoe UI", 9, "bold"), bg=BG, fg=TEXT_DIM).pack(anchor="w", padx=32)

    def _tab(self, nb, title):
        """Crea una pestaña con fondo oscuro y devuelve el frame interior."""
        f = tk.Frame(nb, bg=BG, padx=16, pady=12)
        nb.add(f, text=f"  {title}  ")
        return f

    def _build(self):
        tk.Label(self, text="Configuracion", font=("Segoe UI", 13, "bold"),
                 bg=BG, fg=ACCENT).pack(pady=(18, 8))

        # Estilo del Notebook para que combine con el tema oscuro
        ...
        # ── Pestaña 1: Temporizador ──────────────────────────────────────────
        t1 = self._tab(nb, "⏱ Temporizador")
        frame = tk.Frame(t1, bg=BG2, highlightthickness=1, highlightbackground=BORDER)
        ...
        # ── Pestaña 2: Opciones ──────────────────────────────────────────────
        t2 = self._tab(nb, "⚙ Opciones")

        tk.Label(t2, text="MODO NO MOLESTAR", font=("Segoe UI", 8, "bold"),
                 bg=BG, fg=TEXT_DIM).pack(anchor="w", pady=(4, 2))
        nm_frame = tk.Frame(t2, bg=BG2, highlightthickness=1, highlightbackground=BORDER)
        nm_frame.pack(fill="x")
        ...
        tk.Checkbutton(nm_frame, text="  No interrumpir si hay pantalla completa",
                       variable=self.v_nm, font=("Segoe UI", 9), bg=BG2, fg=TEXT,
                       selectcolor=BG3, activebackground=BG2, activeforeground=TEXT).pack(anchor="w", padx=10, pady=5)
        tk.Checkbutton(nm_frame, text="  Pausar en fin de semana (sab y dom)",
                       variable=self.v_fs, font=("Segoe UI", 9), bg=BG2, fg=TEXT,
                       selectcolor=BG3, activebackground=BG2, activeforeground=TEXT).pack(anchor="w", padx=10, pady=(0, 5))

        tk.Label(t2, text="RECORDATORIO DE AGUA", font=("Segoe UI", 8, "bold"),
                 bg=BG, fg=TEXT_DIM).pack(anchor="w", pady=(12, 2))
        agua_frame = tk.Frame(t2, bg=BG2, highlightthickness=1, highlightbackground=BORDER)
        agua_frame.pack(fill="x")
        ...
        tk.Checkbutton(agua_frame, text="  Activar recordatorio de hidratacion",
                       variable=self.v_agua, font=("Segoe UI", 9), bg=BG2, fg=TEXT,
                       selectcolor=BG3, activebackground=BG2, activeforeground=TEXT).pack(anchor="w", padx=10, pady=5)
        row_agua = tk.Frame(agua_frame, bg=BG2); row_agua.pack(fill="x", padx=10, pady=(0, 8))
        tk.Label(row_agua, text="Cada cuantos minutos:", font=("Segoe UI", 9),
                 bg=BG2, fg=TEXT_DIM).pack(side="left")
        tk.Entry(row_agua, textvariable=self.v_agua_min, font=("Segoe UI", 10),
                 bg=BG3, fg=TEXT, insertbackground=TEXT, relief="flat", bd=0, width=5,
                 highlightthickness=1, highlightbackground=BORDER).pack(side="left", padx=8)

        tk.Label(t2, text="GENERAL", font=("Segoe UI", 8, "bold"),
                 bg=BG, fg=TEXT_DIM).pack(anchor="w", pady=(12, 2))
        self.v_snd  = tk.BooleanVar(value=self.cfg["sonido"])
        self.v_auto = tk.BooleanVar(value=get_autoarranque())
        gen_frame = tk.Frame(t2, bg=BG2, highlightthickness=1, highlightbackground=BORDER)
        gen_frame.pack(fill="x")
        for var, txt in [(self.v_snd,  "  Activar sonido de alerta al iniciar pausa"),
                         (self.v_auto, "  Iniciar con Windows (autoarranque)")]:
            tk.Checkbutton(gen_frame, text=txt, variable=var, font=("Segoe UI", 9),
                           bg=BG2, fg=TEXT, selectcolor=BG3,
                           activebackground=BG2, activeforeground=TEXT).pack(anchor="w", padx=10, pady=4)

        # ── Pestaña 3: Sonido ────────────────────────────────────────────────
        t3 = self._tab(nb, "🔊 Sonido")
        tk.Label(t3, text="SONIDO AMBIENTE DURANTE LA PAUSA",
                 font=("Segoe UI", 8, "bold"), bg=BG, fg=TEXT_DIM).pack(anchor="w", pady=(4, 6))
        amb_frame = tk.Frame(t3, bg=BG2, highlightthickness=1, highlightbackground=BORDER)
        amb_frame.pack(fill="x")
        ...
        for val, lbl in [("ninguno", "Sin sonido"), ("lluvia", "Lluvia"), ("naturaleza", "Naturaleza")]:
            tk.Radiobutton(amb_frame, text=f"  {lbl}", variable=self.v_amb, value=val,
                           font=("Segoe UI", 10), bg=BG2, fg=TEXT, selectcolor=BG3,
                           activebackground=BG2, activeforeground=TEXT).pack(anchor="w", padx=12, pady=6)

        # ── Pestaña 4: Ejercicios ────────────────────────────────────────────
        t4 = self._tab(nb, "🏃 Ejercicios")
        tk.Label(t4, text="Marca los ejercicios que quieres incluir:",
                 font=("Segoe UI", 9), bg=BG, fg=TEXT_DIM).pack(anchor="w", pady=(0, 6))
        ef = tk.Frame(t4, bg=BG2, highlightthickness=1, highlightbackground=BORDER)
        ef.pack(fill="x")
        self.ej_vars = {}
        ...
        for ej in EJERCICIOS:
            ...
            r = tk.Frame(ef, bg=BG2); r.pack(fill="x", padx=10, pady=3)
        ...
        self.lbl_err.pack()
        tk.Button(self, text="  Guardar  ", font=("Segoe UI", 10, "bold"),
                  bg=ACCENT, fg="white", bd=0, cursor="hand2",
                  activebackground="#5A52D5", activeforeground="white",
                  relief="flat", padx=24, pady=8, command=self._save).pack(pady=(4, 16))
    ...
    def _save(self):
        try:
            ...
            assert iv > 0 and dv > 0 and pv > 0 and mv > 0 and amv > 0
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._nm_skip   = False   # flag para cuando se pospuso por no-molestar
    ...
    def _update_tray(self):
        ...
        if not self.running:
            c = TRAY_PAUSED; tip = "Pausado"
        elif not in_active_hours(self.cfg):
            c = TRAY_OFF; tip = "Fuera de horario"
        else:
            c = TRAY_ACTIVE; tip = f"Proxima pausa en {fmt_time(self.remaining)}"
        self._tray.icon  = make_tray_icon(c)
    ...
    def _build(self):
        ...
        self.lbl_agua.pack()
        self._update_cfg_label(); self._update_stats_label(); self._update_agua_label()
        bf = tk.Frame(self, bg=BG); bf.pack(pady=(14, 8))
        self.btn_p = tk.Button(bf, text="Pausar", font=("Segoe UI", 10), bg=BG3, fg=TEXT, bd=0,
                               cursor="hand2", activebackground=BORDER, activeforeground=TEXT,
                               relief="flat", padx=14, pady=7, command=self._toggle)
        self.btn_p.pack(side="left", padx=4)
        tk.Button(bf, text="Pausa ya", font=("Segoe UI", 10), bg=ACCENT, fg="white", bd=0,
                  cursor="hand2", activebackground="#5A52D5", activeforeground="white",
                  relief="flat", padx=14, pady=7, command=self._now).pack(side="left", padx=4)
        tk.Button(bf, text="Posponer", font=("Segoe UI", 10), bg=BG3, fg=TEXT, bd=0,
                  cursor="hand2", activebackground=BORDER, activeforeground=TEXT,
                  relief="flat", padx=14, pady=7, command=self._posponer).pack(side="left", padx=4)
        tk.Button(self, text="Minimizar a bandeja", font=("Segoe UI", 8), bg=BG, fg=TEXT_DIM,
                  bd=0, cursor="hand2", activebackground=BG, activeforeground=TEXT,
                  relief="flat", command=self._hide).pack(pady=(0, 18))
    ...
    def _update_agua_label(self):
        if self.cfg.get("agua_activo", True):
            self.lbl_agua.config(text=f"💧 Recordatorio de agua cada {self.cfg.get('agua_min', 30)} min")
        else:
    ...
    def _tick(self):
        ...
        # MEJORA 4 — Fin de semana
        if self.cfg.get("fin_de_semana", False) and is_weekend():
        ...
        if self.remaining <= 0:
            # MEJORA 1 — No molestar
            if self.cfg.get("no_molestar", True) and is_fullscreen_app_running():
                self.lbl_st.config(text="Pausa pospuesta (pantalla completa detectada)")
                self.lbl_badge.config(text="NO MOLESTAR", fg=YELLOW)
                self._total_sec = 5 * 60   # reintenta en 5 min
    ...
    def _trigger_pausa(self):
        ...
        threading.Thread(
            target=lambda: send_win_notification("Pausa Activa", "Es hora de moverte un poco!"),
            daemon=True).start()
    ...
    def _done_pausa(self):
        ...
        if self.stats["completadas"] == meta:
            ...
            threading.Thread(
                target=lambda: send_win_notification("Meta cumplida!", f"Completaste {meta} pausas hoy. Excelente habito!"),
                daemon=True).start()
    ...
    def _open_config(self):
        def on_save(c):
            ...
            # Reinicia el recordatorio de agua con la nueva config
            self._water.restart()
